...
public class PDFWriter {
    ...
    public void setColor(double r, double g, double b) {
        ...
        mCurrentPage.addRawContent("[] 0 d\n");
        mCurrentPage.addRawContent("1 w\n");
        mCurrentPage.addRawContent(String.valueOf(rp)+" "+String.valueOf(gp)+" "+String.valueOf(bp)+" RG\n");
        mCurrentPage.addRawContent(String.valueOf(rp)+" "+String.valueOf(gp)+" "+String.valueOf(bp)+" rg\n");
    }

	public void addText(int x, int y, int fontSize, String text) {
        y = pageHeight - y - fontSize;
		addText(x, y, fontSize, text, Transformation.DEGREES_0_ROTATION);
	}

    public int addTextMultiline(int x, int y, int fontSize, int caracters, String text) {
        y = pageHeight - y - fontSize;
        int iteration=0;
        while(text.length()>0){
            String print;
            if(text.length()>caracters) {
                print = text.substring(0, caracters);
                int space = print.lastIndexOf(" ");
                if(space>0 && space > ((caracters/4)*3)){
                    print = text.substring(0, space);
                }
                text=text.substring(print.length(),text.length());
            } else {
                print = text.substring(0,text.length());
                text="";
            }
            addText(x, y-(fontSize*iteration), fontSize, print, Transformation.DEGREES_0_ROTATION);
            iteration++;
        }
        return fontSize*iteration+3;
    }

    public void addTextCorrect(int x, int y, int fontSize, String text, String transformation) {
        y = pageHeight - y - fontSize;
        addText(x, y, fontSize, text, transformation);
    }
	...
	public void addLine(int x, int y, int x2, int y2) {
        y = pageHeight - y;
        y2 = pageHeight - y2;
		mCurrentPage.addLine(x, y, x2, y2);
	}
	
	public void addRectangle(int x, int y, int w, int h) {
        y = pageHeight - y - h;
		mCurrentPage.addRectangle(x, y, w, h);
	}
	...
	public void addImage(int x, int y, int w, int h, Bitmap bitmap) {
        y = pageHeight - y - h;
		addImage(x, y, w, h, bitmap, Transformation.DEGREES_0_ROTATION);
	}
